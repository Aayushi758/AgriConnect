"""Synthetic data generation for AgriConnect prototype."""
import random
import math
...
def seed_database(db: Session):
    ...
    for i, name in enumerate(FARMER_NAMES):
        ...
        user = User(
            email=f"farmer{i+1}@agriconnect.com",
    ...
    for i, name in enumerate(CONSUMER_NAMES):
        loc = LOCATIONS[i % len(LOCATIONS)]
        user = User(
            email=f"consumer{i+1}@agriconnect.com",
    ...
    for farmer in farmers:
        ...
        for crop in farmer_crops:
            ...
            img_url = CROP_IMAGES.get(crop["name"], DEFAULT_IMAGE)
            pi = ProductImage(product_id=product.id, image_url=img_url, is_primary=True)
            db.add(pi)
